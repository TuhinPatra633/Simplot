...
def prompt(args, index, suppress_errors=False):
    gemini_api_key = args.api_key
    if not gemini_api_key:
        raise ValueError("[ERROR] Gemini API key is missing. Please provide it as a script argument or set it in the environment.")
    ...
    try:
        ...
        json_path = os.path.join(args.json_path, img[:-3] + 'json')
        ...
        table_data = args.pred[args.pred['img'] == img]['md'].values
        if len(table_data) == 0:
            raise ValueError(f"No table data found for image: {img}")
        table_data = table_data[0]

        question = args.qa[index]['query']
        if not question:
            raise ValueError(f"Missing question for QA entry at index {index}")

        # Define LangChain models and prompt
        chat_model = ChatGoogleGenerativeAI(google_api_key=gemini_api_key, model="gemini-1.5-pro-latest")
        chat_template = ChatPromptTemplate.from_messages([
            SystemMessage(content="You are a helpful assistant. Given a table and a question, answer based on the table."),
            HumanMessagePromptTemplate.from_template("Here is a table:\n{table}\n\nHere is a question:\n{question}\n\nAnswer:")
        ])

        # Generate the answer
        prompt_data = chat_template.format_messages(table=table_data, question=question)
        response = chat_model(prompt_data)
        
        # Process the response
        answer = response.content.strip()
        correct = answer == args.qa[index]['label']
        return {
            'index': index,
            'img': img,
            'type': data_json['type'],
